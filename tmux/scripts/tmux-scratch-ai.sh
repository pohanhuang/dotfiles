...
# Define AI scratchpad session name (per-directory)
session="_popup_ai_${dir_name}"
...
if ! tmux has -t "$session" 2>/dev/null; then
	...
	# Set a welcome message for AI pad
	tmux send-keys -t "$session_id" "# AI Scratch Pad - $(basename $current_dir)" C-m
	tmux send-keys -t "$session_id" "# Use this for: prompts, AI conversations, ideas" C-m
	tmux send-keys -t "$session_id" "" C-m

	session="$session_id"
fi

# Attach to the AI scratchpad session inside the popup
